...
echo "$($PSQL "SELECT DISTINCT teams.name FROM teams LEFT JOIN games ON teams.team_id = games.winner_id WHERE winner_id IS NOT NULL ORDER BY name")"

echo -e "\nYear and team name of all the champions:"
echo "$($PSQL "SELECT games.year, teams.name FROM teams FULL JOIN games on teams.team_id = games.winner_id WHERE winner_id IS NOT NULL AND round = 'Final' ORDER BY year")"
